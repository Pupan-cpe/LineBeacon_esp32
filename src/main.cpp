...
int i;
static const uint8_t HWID[5] = {0x01, 0x48, 0x5e, 0xac, 0xf6};// ผูก Hardware ID

static const uint8_t ADC_PIN = 35;  // 電圧入力ピン 0xFE6F
static const float R1 = 4000.0;
static const float R2 = 2000.0;
static const uint16_t Major_FOR_LINECORP = 	0x4C49;  //i beacon major for ios
  ...
  uint8_t* dataPtr = data;
{
  ...
  }
  {
    ...
    putUint16LE(&dataPtr, Major_FOR_LINECORP);
  }
  {
    // simple beacon
...
Serial.printf("analogRead: %d,  voltage: %f", analog12bit, voltage);
  Serial.println();
char dm[] = "Hello World"; // ส่งข้อความเข้า Channel ของเรา
